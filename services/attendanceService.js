...
export default class AttendanceService {
    ...
    async registerAttendance(attendanceData){
        ...
        const attendances = await this.attendanceDAO.getByCourse(attendanceData.course);
        ...
        if(!attendances){
            ...
            course.students = attendanceData.students.map(student => student.student);
        }
    ...
    async getAttendanceByCourse(courseId){
        const attendance = await this.attendanceDAO.getByCourse(courseId);
        return attendance;
    }
}